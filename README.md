...
All commands below are run from this repository root. The old `muse-labeling/`
subfolder has been flattened into the root project directory.
...
```bash
cd /Users/aurora/Desktop/Muse/label_tool/openhub_muse_label_tool
npm install
npm run dev
```
...
`npm run build` only creates the production files in `dist/`; it does not start
the labeling tool.

### Method 2: Docker

Docker Desktop must be installed and running first. If `docker` is not found,
install Docker Desktop and reopen the terminal before running these commands.
...
Ignored local/generated folders such as `node_modules/`, `dist/`, and
`my_output_frames*/` may exist in the working tree but are not committed.
...
```bash
python3 split_label_json.py /Users/aurora/Desktop/my_output_frames_day_1/yolo_tracking_data.json \
  --frames-per-file 500
```
...
Relaxed labeling export settings are mirrored in this repository as
`export_label_json_relaxed.py`. This helper depends on the MUSE processing
modules (`cfar.py`, `radar_processing.py`, `tracking.py`, and `utils.py`), so
copy or sync it into the MUSE `Processing/` environment before generating a new
export from radar/camera data:

```bash
python3 export_label_json_relaxed.py \
...
```bash
python3 prelabel_object.py yolo_tracking_data.json --radar-id 1 --box-id 1
```
